    ...
    def _to_pos(self, X):
        ...
        for track in tqdm(X):
            ...
            for joint in track.traverse():
                ...
                pos_df = pd.concat([pos_df, pd.Series(data=[e[0] for e in tree_data[joint][1]], index=pos_df.index,
                                                      name='%s_Xposition' % joint)], axis=1)
                pos_df = pd.concat([pos_df, pd.Series(data=[e[1] for e in tree_data[joint][1]], index=pos_df.index,
                                                      name='%s_Yposition' % joint)], axis=1)
                pos_df = pd.concat([pos_df, pd.Series(data=[e[2] for e in tree_data[joint][1]], index=pos_df.index,
                                                      name='%s_Zposition' % joint)], axis=1)
...
class Mirror(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            ryp = '%s_Yposition' % track.root_name
            rzp = '%s_Zposition' % track.root_name
            new_df = pd.concat([new_df, pd.Series(data=-signs[0] * euler_df[rxp], index=new_df.index, name=rxp)],
                               axis=1)
            new_df = pd.concat([new_df, pd.Series(data=-signs[0] * euler_df[ryp], index=new_df.index, name=ryp)],
                               axis=1)
            new_df = pd.concat([new_df, pd.Series(data=-signs[0] * euler_df[rzp], index=new_df.index, name=rzp)],
                               axis=1)
            ...
            for lft_joint in lft_joints:
                ...
                # Create the corresponding columns in the new DataFrame
                new_df = pd.concat([new_df, pd.Series(data=signs[0] * track.values['%s_Xrotation' % rgt_joint],
                                                      index=new_df.index, name='%s_Xrotation' % lft_joint)], axis=1)
                new_df = pd.concat([new_df, pd.Series(data=signs[1] * track.values['%s_Yrotation' % rgt_joint],
                                                      index=new_df.index, name='%s_Yrotation' % lft_joint)], axis=1)
                new_df = pd.concat([new_df, pd.Series(data=signs[2] * track.values['%s_Zrotation' % rgt_joint],
                                                      index=new_df.index, name='%s_Zrotation' % lft_joint)], axis=1)
                ...
                new_df = pd.concat([new_df, pd.Series(data=signs[0] * track.values['%s_Xrotation' % lft_joint],
                                                      index=new_df.index, name='%s_Xrotation' % rgt_joint)], axis=1)
                new_df = pd.concat([new_df, pd.Series(data=signs[1] * track.values['%s_Yrotation' % lft_joint],
                                                      index=new_df.index, name='%s_Yrotation' % rgt_joint)], axis=1)
                new_df = pd.concat([new_df, pd.Series(data=signs[2] * track.values['%s_Zrotation' % lft_joint],
                                                      index=new_df.index, name='%s_Zrotation' % rgt_joint)], axis=1)
            ...
            for joint in joints:
                ...
                new_df = pd.concat([new_df,
                                    pd.Series(data=signs[0] * track.values['%s_Xrotation' % joint], index=new_df.index,
                                              name='%s_Xrotation' % joint)], axis=1)
                new_df = pd.concat([new_df,
                                    pd.Series(data=signs[1] * track.values['%s_Yrotation' % joint], index=new_df.index,
                                              name='%s_Yrotation' % joint)], axis=1)
                new_df = pd.concat([new_df,
                                    pd.Series(data=signs[2] * track.values['%s_Zrotation' % joint], index=new_df.index,
                                              name='%s_Zrotation' % joint)], axis=1)
...
class EulerReorder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            new_df = pd.concat([new_df,
                                pd.Series(data=euler_df[rxp], index=new_df.index,
                                          name=rxp)], axis=1)
            new_df = pd.concat([new_df,
                                pd.Series(data=euler_df[ryp], index=new_df.index,
                                          name=ryp)], axis=1)
            new_df = pd.concat([new_df,
                                pd.Series(data=euler_df[rzp], index=new_df.index,
                                          name=rzp)], axis=1)
            ...
            for joint in joints:
                ...
                # Create the corresponding columns in the new DataFrame
                new_df = pd.concat([new_df,
                                    pd.Series(data=[e[0] for e in new_euler], index=new_df.index,
                                              name='%s_%srotation' % (joint, self.new_order[0]))], axis=1)
                new_df = pd.concat([new_df,
                                    pd.Series(data=[e[1] for e in new_euler], index=new_df.index,
                                              name='%s_%srotation' % (joint, self.new_order[1]))], axis=1)
                new_df = pd.concat([new_df,
                                    pd.Series(data=[e[2] for e in new_euler], index=new_df.index,
                                              name='%s_%srotation' % (joint, self.new_order[2]))], axis=1)
