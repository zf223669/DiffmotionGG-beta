...
class EpsilonTheta(nn.Module):
    ...
    def forward(self, inputs, time, cond):
        # logger.info('-------------------EpsilonTheta forward-------------------')
        ...
        skip = []
        residual_layer_count = 1;
        for layer in self.residual_layers:
            # logger.info(f'residual_layer_count: {residual_layer_count}')
            residual_layer_count += 1
        ...
        # logger.info(f'EpsilonTheta x.shape{x.shape}')
        return x
